...
export class PhoneBookComponent implements OnInit {
  ...
  filter_users = () => {
    if(this.filterdata){
      this.filter_array=this.User_data.filter((user:PhoneBook)=>{
        return user.firstName.toLowerCase().indexOf(this.filterdata.toLowerCase())>-1||user.lastName.toLowerCase().indexOf(this.filterdata.toLowerCase())>-1
        ||user.phoneNumber.toString().toLowerCase().indexOf(this.filterdata.toLowerCase())>-1
      })
    }
  ...
  handleSort=()=>{
    if(this.sort_by==='firstName'){
      ...
      this.filter_array=this.filter_array.sort((a,b)=>(a.phoneNumber-b.phoneNumber));
    }
